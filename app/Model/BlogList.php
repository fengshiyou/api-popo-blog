...
class BlogList extends Model
{
    ...
    public function scopeGetCatalog($query){
        $query = $query->addSelect(DB::raw('group_concat(catalog2.catalog_name,"|",catalog2.id,"|",catalog2.parend_id) as catalog'));
        ...
        $query = $query->leftJoin('catalog as catalog2',function ($join){
            $join->on('catalog2.lef','<=','catalog1.lef')->on('catalog2.rig','>=','catalog1.rig');
        });
